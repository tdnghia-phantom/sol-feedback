...
3. Trong editor, chọn hàm **`setupFeedbackSheet`** → Run → cấp quyền khi được hỏi.
   ...
   **Xem hộp thoại/Executions log để lấy passcode admin mẫu** (dạng `ADMIN-FB…`).
   Chọn tiếp hàm **`setupFeedbackPageList`** → Run → tạo tab **`feedback-page-list`** (4 cột:
   `slug·label·active·note`) + nạp sẵn 2 dòng: `ckry` (SOL Cookery) và `test` (trang test) — đều BẬT.

...
6. **Bật/tắt trang feedback:** Quản lý → **Trang Feedback** → mỗi trang có nút **Bật/Tắt** (có **popup xác nhận**;
   xác nhận xong **bắn Telegram** cho admin). **Tắt** một trang ⇒ phụ huynh mở link **bị redirect về
   `https://fbk.solenglishland.vn/`**; **Bật** lại là dùng ngay. Danh sách quản lý bằng sheet
   **`feedback-page-list`**; mỗi dòng hiện sẵn **đường dẫn `?sw=…`** để copy đi in QR/gửi Zalo.
   Thêm trang mới: **+ Tạo mới** (slug phải khớp `?sw=` trên link) → bắn Telegram.
   *(Có sẵn trang **`?sw=test`** — chỉ chữ "test" giữa màn hình — để bạn thử bật/tắt + redirect.)*
   **Lịch tự động:** trong popup Tạo/Sửa có 2 ô **🟢 Tự BẬT lúc** / **🔴 Tự TẮT lúc** (giờ VN, tùy chọn).
   Đến giờ hẹn, hệ thống **tự bật/tắt** (trigger `autoTogglePages` chạy **mỗi 1 tiếng**) và **bắn Telegram** cho bạn.
   Để trống = không tự động (chỉ bật/tắt tay). *(Đặt giờ ở tương lai; giờ quá khứ sẽ không kích hoạt. Vì kiểm tra
   mỗi 1 tiếng nên thời điểm tự bật/tắt có thể trễ tối đa ~1 tiếng so với giờ hẹn.)*

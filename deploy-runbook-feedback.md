...
3. Trong editor, chọn hàm **`setupFeedbackSheet`** → Run → cấp quyền khi được hỏi.
   ...
   **Xem hộp thoại/Executions log để lấy passcode admin mẫu** (dạng `ADMIN-FB…`).
   Chọn tiếp hàm **`setupWorkshopsSheet`** → Run → tạo tab **`workshops`** (4 cột:
   `ws_id·label·active·note`) + nạp sẵn dòng `cookery = TRUE` (đang mở).
...
2. Đăng nhập bằng **passcode admin mẫu** lấy ở Bước 1.3 (`ADMIN-FB…`).
   → Vào được 4 tab: **💬 Cảm nhận** · **📊 Dashboard** · **🗂️ Trang feedback** · **👤 Nhân viên**.
...
6. **Đóng/mở trang feedback:** tab **🗂️ Trang feedback** → mỗi workshop có nút **Đóng/Mở**.
   Đóng `ckry` ⇒ phụ huynh mở `feedback.sol.vn/?sw=ckry` sẽ thấy thông báo *"đã đóng"*
   và không gửi được (chặn cả ở trang lẫn ở server). Mở lại là dùng ngay. Thêm workshop mới
   vào danh sách bằng form (ws_id phải khớp `?sw=` trên link). Bảng có sẵn **đường dẫn đầy đủ**
   để copy đi in QR/gửi Zalo.

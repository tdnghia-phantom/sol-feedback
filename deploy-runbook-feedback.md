...
3. Trong editor, chọn hàm **`setupFeedbackSheet`** → Run → cấp quyền khi được hỏi.
   ...
   **Xem hộp thoại/Executions log để lấy passcode admin mẫu** (dạng `ADMIN-FB…`).
   ...
   Chọn tiếp hàm **`installAutoToggleTrigger`** → Run → cài lịch tự bật/tắt (kiểm tra mỗi 5 phút).
...
6. **Bật/tắt trang feedback:** Quản lý → **Trang Feedback** → mỗi trang có nút **Bật/Tắt** (có **popup xác nhận**;
   ...
   **Lịch tự động:** trong popup Tạo/Sửa có 2 ô **🟢 Tự BẬT lúc** / **🔴 Tự TẮT lúc** (giờ VN, tùy chọn).
   Đến giờ hẹn, hệ thống **tự bật/tắt** (trigger `autoTogglePages` chạy mỗi 5') và **bắn Telegram** cho bạn.
   Để trống = không tự động (chỉ bật/tắt tay). *(Đặt giờ ở tương lai; giờ quá khứ sẽ không kích hoạt.)*
